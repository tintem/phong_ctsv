...
class Result extends CI_Controller {
	...
	public function index($limit='0',$status='0')
	{
		...
		if($logged_in['su']=='1')
		{
			...
			$data['result']=$this->result_model->new_result();
			$this->load->view('result_list2',$data);	
		}
		else
		{
			$data['result']=$this->result_model->result_list_student();
	...
	public function filter()
	{
		...
		$data['result']=$this->result_model->result_filter();
		// fetching quiz list
		$data['quiz_list']=$this->result_model->quiz_list();
		// group list
		$this->load->model("user_model");
	...
	function generate_report(){
		...
		$csvdata="Ma bai thi,Email,Ho va ten,Ten nhom,Ten bai thi,So cau dat duoc,Phan tram dat duoc,Ket qua\r\n";
		foreach($result as $rk => $val){
		$csvdata.=$val['rid'].",".$val['email'].",".$val['last_name']." ".$val['first_name'].",".$val['group_name'].",".$val['quiz_name'].",".$val['score_obtained'].",".$val['percentage_obtained'].",".$val['result_status']."\r\n";
		}
		$filename=time().'.csv';
