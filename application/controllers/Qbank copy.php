...
class Qbank extends CI_Controller {
	...
	public function index($limit='0',$cid='0',$lid='1')
	{
	...
	//	$data['result']=$this->qbank_model->question_list($limit,$cid,$lid);
		$data['result']=$this->qbank_model->question_list(0,0,0);
	...
	public function new_question_1($nop='4')
	{
			...
			$this->form_validation->set_rules('question', 'Question', 'is_unique[savsoft_users.email]');
			$this->form_validation->set_rules('option[0]', 'Option 1', 'required');
			$this->form_validation->set_rules('option[1]', 'Option 2', 'required');
			$this->form_validation->set_rules('option[2]', 'Option 3', 'required');
			$this->form_validation->set_rules('option[3]', 'Option 4', 'required');
			
			if ($this->form_validation->run() == FALSE)
			{
				$this->session->set_flashdata('message', "<div class='alert alert-danger'>".validation_errors()." </div>");
				redirect('qbank/new_question_1/');
			}else
			{
				...
				redirect('qbank/new_question_1/4');
			}			
		}
		 $data['nop']=$nop;
		 $data['title']=$this->lang->line('add_new');
		// fetching category list
		$data['category_list']=$this->qbank_model->category_list('asc');
		// fetching level list
		$data['level_list']=$this->qbank_model->level_list();
		 $this->load->view('header',$data);
